...
response = requests.get(api_url + counURL, headers={'X-Api-Key': 'lrpYAlfNcKMuN4f+W6OQvw==uXlsnR78sBXBiXRr'})
if response.status_code == requests.codes.ok:
    lat = response.json()[0]['latitude']
    long = response.json()[0]['longitude']
    wet_url = 'https://api.openweathermap.org/data/2.5/weather?lat={latitude}&lon={longitude}&appid={APIKEY}'.format(latitude=lat,longitude=long,APIKEY="6c0326a1876663a3cf6e4137d810ace0")
